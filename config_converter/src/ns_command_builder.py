...
COPIED VERBATIM from ``template_converter/src/ns_command_builder.py``. The
ONLY line touched when copying this file into ``config_converter`` is the
``stencil_mapper`` import below. The body is identical across every
converter in this repo: any ``NSModel``, built by any platform's mapper,
serialises the same way -- see ``config_converter/DESIGN.md`` section 1.1 for
why this file must NOT be customised.
...
# and unified across every converter in this repo. Do NOT invent new colours;
# reuse this exact palette (see GUIDE.md "Device colour convention").
...
def cmd_add_l1_link_bulk(model: NSModel) -> Optional[str]:
    ...
    Generic serialiser only: every entry in ``model.l1_links`` becomes one
    row regardless of how the mapper derived it (subnet-matched pair,
    real-peer Port-channel member pairing, or an inferred ``Dummy_<TC>_<n>``
    device's per-member link when a Port-channel's peer is entirely absent
    from the input corpus -- see ``config_converter/DESIGN.md`` section
    4.3.6). The mapper (topology_mapper.py) is responsible for expanding a
    Port-channel into one NSL1Link per physical member before this function
    ever runs; no Port-channel-specific branching belongs here.
    """
...
def cmd_rename_port_info_bulk(model: NSModel, unknown: bool = False) -> Optional[str]:
    ...
    When ``unknown`` is True, every port is set to Unknown/Unknown/Unknown.
    """
    if not model.devices:
        return None
    devices_with_l1_link: set = set()
    for lk in model.l1_links:
        devices_with_l1_link.add(lk.a_device)
        devices_with_l1_link.add(lk.b_device)
    groups: Dict[Tuple[str, str, str], List[str]] = {}
    for d in model.devices.values():
        if d.stencil.stencil_type == NS_CLOUD:
            continue
        if d.name not in devices_with_l1_link:
            # A device with zero L1 links has no ports known to the live
            # engine yet (see topology_mapper.apply_parsed_configs's
            # devices_with_l1_link skip-note for the full rationale), so a
            # `_ALL_` port_info rename against it would only ever produce a
            # harmless-but-noisy "not found" result. Skip it here instead.
            continue
        if unknown:
...
def cmd_add_portchannel_bulk(model: NSModel) -> Optional[str]:
    ...
    Generic serialiser only: renders whatever ``NSPortChannel.physical_ports``
    the mapper populated for ``pc.device``, independent of whether the peer
    end was a real device (topology_mapper.py's member-pairing algorithm,
    DESIGN.md 4.3.6) or an inferred ``Dummy_<TC>_<n>`` device created because
    the peer Port-channel/device was entirely absent from the input corpus
    (same section -- every local member still gets its own inferred peer
    interface and its own NSL1Link, never a single collapsed representative
    link). No config_converter-specific logic belongs here.
    """
...
def cmd_rename_attribute_bulk(model: NSModel) -> str:
    ...
    for name, d in sorted(model.devices.items()):
        ...
        # A per-device override wins over the role-based colour; the token
