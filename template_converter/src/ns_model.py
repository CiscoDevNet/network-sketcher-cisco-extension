...
TEMPLATE FILE — copy this into your new converter's ``src/`` UNCHANGED.
This module is the converter-agnostic half of the Network Sketcher pipeline,
shared verbatim across aci_converter / catc_converter / nd_converter /
3rd_party/netbox_converter (and now this template) so that every mapper
(physical / logical / whatever your platform needs) emits the *same*
``NSModel`` and the single ``ns_command_builder`` can serialise any of them
identically. The ONLY line you should ever need to touch when copying this
file is the ``platform_stencil_mapper`` import below — repoint it at your new
converter's own stencil-mapper module name.

It contains ONLY the reusable pieces — no platform-specific build logic:
...
from .platform_stencil_mapper import StencilMapping
