...
lifted verbatim from ``cml_converter/src/topology_mapper.py`` so that both ACI
mappers (physical / logical) emit the *same* ``NSModel`` and the single
``ns_command_builder`` can serialise either one identically.

It contains ONLY the reusable pieces — no ACI- or CML-specific build logic:
...
from .meraki_stencil_mapper import StencilMapping
...
def normalise_port_name(raw: str) -> str:
    ...
    Examples:
        ...
        eth1/49                -> Ethernet 1/49     (ACI fabric ports)
...
def build_area_layout(
    ...
    # Devices uplinked to a waypoint (Internet/WAN/cloud) area become the top of
    # their own area's hierarchy, regardless of stencil tier.
    uplink_roots: Set[str] = set()
    for lk in (l1_links or []):
        a, b = lk.a_device, lk.b_device
        if a in devices and b in devices:
            # areas are already rendered here, so waypoints carry the '_wp_'
            # suffix (raw names are kept too for safety).
            a_wp = devices[a].area.endswith("_wp_") or devices[a].area in _RAW_WAYPOINT_AREAS
            b_wp = devices[b].area.endswith("_wp_") or devices[b].area in _RAW_WAYPOINT_AREAS
            if a_wp and not b_wp:
                uplink_roots.add(b)
            elif b_wp and not a_wp:
                uplink_roots.add(a)
    ...
    for orig_area, devs in by_area.items():
        ...
        if grid is None:
            # Tier (row) per device comes from the L1 wiring (BFS depth from the
            # area's upstream roots), so the hierarchy deepens automatically
            # (FW -> router -> core -> distribution -> access -> hosts) instead
            # of a fixed per-role table. Devices the same hop-count from a root
            # share a row (e.g. an AP and a camera both off one switch).
            depth = _depth_tiers(devs, adjacency, uplink_roots)
            row_buckets: Dict[int, List[str]] = {}
            for d in devs:
                row_buckets.setdefault(depth[d.name], []).append(d.name)
            tier_rows: List[List[str]] = [
                sorted(row_buckets[t]) for t in sorted(row_buckets.keys())
            ]
            grid = _place_tree_columns(tier_rows, adjacency)
        area_to_grid[rendered] = grid

    # Stack waypoint (Internet / WAN / cloud) areas on a row ABOVE the site
    # areas, so external connectivity sits at the very top of the diagram.
    wp_row = [name_map[a] for a in ordered_areas if a in _RAW_WAYPOINT_AREAS]
    main_row = [name_map[a] for a in ordered_areas if a not in _RAW_WAYPOINT_AREAS]
    area_layout = [row for row in (wp_row, main_row) if row]
...
def _depth_tiers(
    ...
    uplink_roots: Optional[Set[str]] = None,
) -> Dict[str, int]:
    """Tier (row index) per device from the L1 wiring within one area.

    Roots (top row) are, in order of preference: the devices uplinked to a
    waypoint/Internet area (`uplink_roots`); otherwise the devices on the lowest
    RULE 0 stencil tier present in the area (e.g. the core switch in a
    switch-only area). Every other device's tier is its BFS hop-count from the
    nearest root, so the hierarchy deepens automatically with the topology
    (FW -> router -> core -> distribution -> access -> hosts). Devices not
    reachable from any root (no synthesized/real link) fall back to their
    stencil order, stacked below the reachable tree.
    """
    names = {d.name for d in devs}
    by_name = {d.name: d for d in devs}
    adj = {n: sorted(x for x in adjacency.get(n, ()) if x in names) for n in names}

    roots = sorted((uplink_roots or set()) & names)
    if not roots:
        min_row = min(d.row for d in devs)
        roots = sorted(d.name for d in devs if d.row == min_row)
    ...
    if len(depth) < len(names):
        ...
        leftover = sorted(
            (by_name[n] for n in names if n not in depth),
            key=lambda d: (d.row, d.name),
        )
        row_to_tier: Dict[int, int] = {}
        for d in leftover:
            if d.row not in row_to_tier:
                row_to_tier[d.row] = max_d + 1 + len(row_to_tier)
            depth[d.name] = row_to_tier[d.row]
    return depth


def _place_tree_columns(
    tier_rows: List[List[str]],
    adjacency: Dict[str, Set[str]],
) -> List[List[str]]:
    """Place devices tier-by-tier so each sits directly below the parent(s) it
    connects to in the tier above (tree alignment + RULE 0.5 crossing control).

    A straight chain (FW-router-core-...) collapses to one vertical column; when
    several upstream paths exist each subtree lines up beneath its own parent
    with the shortest, straightest links. Empty grid cells are '_AIR_'.
    """
    # 1) Fix the left-right ORDER within each row (barycentre crossing control).
    ordered = _place_columns([list(r) for r in tier_rows if r], adjacency)
    if not ordered:
        return []

    names = {n for r in ordered for n in r}
    adj = {n: [x for x in adjacency.get(n, ()) if x in names] for n in names}
    pos: Dict[str, float] = {}
    for r in ordered:
        for i, n in enumerate(r):
            pos[n] = float(i)

    def _assign(row: List[str], ref: Set[str]) -> None:
        """Slide each node toward the mean column of its neighbours in `ref`,
        preserving order with a minimum 1-column gap."""
        last = float("-inf")
        for n in row:
            refs = [pos[x] for x in adj[n] if x in ref]
            want = sum(refs) / len(refs) if refs else pos[n]
            pos[n] = max(want, last + 1.0)
            last = pos[n]

    # 2) Alternate top-down (centre children under parents) and bottom-up
    #    (centre parents over children) sweeps until positions settle.
    for _ in range(6):
        for ri in range(1, len(ordered)):
            _assign(ordered[ri], set(ordered[ri - 1]))
        for ri in range(len(ordered) - 2, -1, -1):
            _assign(ordered[ri], set(ordered[ri + 1]))

    # 3) Snap floats onto a shared integer grid; natural gaps become '_AIR_'.
    min_p = min(pos.values())
    grid: List[List[str]] = []
    for r in ordered:
        cells: Dict[int, str] = {}
        last_col = -1
        for n in r:
            c = int(round(pos[n] - min_p))
            if c <= last_col:
                c = last_col + 1
            cells[c] = n
            last_col = c
        row_w = (max(cells) + 1) if cells else 0
        line = ["_AIR_"] * row_w
        for c, n in cells.items():
            line[c] = n
        grid.append(line)

    return _trim_air_columns(grid)


def _trim_air_columns(grid: List[List[str]]) -> List[List[str]]:
    """Drop grid columns that are '_AIR_' in every row (uniform shift keeps
    relative alignment); rows are padded to a common width first."""
    if not grid:
        return grid
    width = max((len(r) for r in grid), default=0)
    padded = [r + ["_AIR_"] * (width - len(r)) for r in grid]
    keep = [c for c in range(width) if any(r[c] != "_AIR_" for r in padded)]
    if not keep:
        return []
    return [[r[c] for c in keep] for r in padded]
