...
This is a sibling copy of ``cml_converter/src/ns_command_builder.py`` (the
"each converter is standalone" repo convention) with the imports pointed at
``.ns_model`` / ``.aci_stencil_mapper``. The body is identical — any NSModel,
whether built by the physical or logical ACI mapper, serialises the same way.
...
from .meraki_stencil_mapper import NS_CLOUD, NS_PC, NS_PHONE, NS_SERVER, StencilMapping


# RULE 16 colour palette — Attribute-sheet 'Default' cell colours, role-based
# and unified with cv_converter / cml_converter / sna_converter.
