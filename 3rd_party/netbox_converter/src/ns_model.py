...
from .netbox_stencil_mapper import StencilMapping
...
def build_area_layout(
    ...
    # Place waypoint areas (names ending '_wp_', e.g. a WAN cloud) on a top row
    # so they sit ABOVE the network-group areas they connect down into, rather
    # than beside them; everything else goes on the row below.
    wp_row = [a for a in rendered_areas if a.endswith("_wp_")]
    main_row = [a for a in rendered_areas if not a.endswith("_wp_")]
    area_layout = [wp_row, main_row] if wp_row else [main_row]
...
def _cross_pair(upper: List[str], lower: List[str],
                adj: Dict[str, List[str]]) -> int:
    """Number of edge crossings between two adjacent rows for the given order."""
...
def _total_crossings(rows: List[List[str]], adj: Dict[str, List[str]]) -> int:
    return sum(_cross_pair(rows[i], rows[i + 1], adj) for i in range(len(rows) - 1))
...
def _place_columns(
    ...
    """Order devices within each tier row to minimise L1 line crossings
    (RULE 0.5) with a Sugiyama-style layered sweep: alternating up/down median
    ordering + adjacent transpose, keeping the order with the fewest crossings.
    Rows themselves are never reordered (RULE 0)."""
    ...
    def _median_pass(ri: int, fixed: int) -> None:
        posf = {n: i for i, n in enumerate(rows[fixed])}
        cur = {n: i for i, n in enumerate(rows[ri])}
        key: Dict[str, float] = {}
        for n in rows[ri]:
            nb = [posf[x] for x in adj[n] if x in posf]
            key[n] = statistics.median(nb) if nb else float(cur[n])
        rows[ri].sort(key=lambda n: (key[n], cur[n]))
            ...
            for ri in range(len(rows)):
                ...
                for i in range(len(row) - 1):
                    def local() -> int:
                        c = 0
                        if ri > 0:
                            c += _cross_pair(rows[ri - 1], rows[ri], adj)
                        if ri < len(rows) - 1:
                            c += _cross_pair(rows[ri], rows[ri + 1], adj)
                        return c
                    before = local()
    ...
    best_cross = _total_crossings(rows, adj)
    for _ in range(_LAYER_ROUNDS):
        for ri in range(1, len(rows)):          # down sweep
            _median_pass(ri, ri - 1)
        for ri in range(len(rows) - 2, -1, -1):  # up sweep
            _median_pass(ri, ri + 1)
        _transpose()
        c = _total_crossings(rows, adj)
        if c < best_cross:
            best_cross = c
            best = [list(r) for r in rows]
        if best_cross == 0:
